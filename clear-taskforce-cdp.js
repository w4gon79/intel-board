...
async function main() {
  ...
  // Clear task force events via IPC
  const r1 = await s('Runtime.evaluate', {
    expression: "(async()=>{await window.api.tactical.clearEvents('task_force');return 'cleared'})()",
    awaitPromise: true
  });
  console.log('CLEAR TASK FORCE:', r1.result?.result?.value);
