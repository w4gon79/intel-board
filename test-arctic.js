...
async function main() {
  ...
  const r1 = await s('Runtime.evaluate', {
    expression: "(async()=>{const g=await window.api.ais.getGeoJSON();const mm=['303829003','366991414','369970758'];const v=g.features.filter(f=>mm.includes(String(f.properties.mmsi)));return JSON.stringify(v.map(f=>({name:f.properties.ship_name,mmsi:f.properties.mmsi,lat:f.geometry.coordinates[1],lon:f.geometry.coordinates[0],hdg:f.properties.heading})))})()",
    awaitPromise: true
  });
  console.log('ARCTIC VESSELS:', r1.result?.result?.value);
